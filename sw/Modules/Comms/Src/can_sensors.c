...
CanCommsStatus_E can_send_ImuData(
  ...
  int16_t frame_num =
      canardBroadcast(&canard, UAVCAN_EQUIPMENT_AHRS_SENSORIMU_SIGNATURE,
                      UAVCAN_EQUIPMENT_AHRS_SENSORIMU_ID, &transfer_id,
...
CanCommsStatus_E can_send_GpsData(
  ...
  int16_t frame_num =
      canardBroadcast(&canard, UAVCAN_EQUIPMENT_GNSS_SENSORGPS_SIGNATURE,
                      UAVCAN_EQUIPMENT_GNSS_SENSORGPS_ID, &transfer_id,
                      CANARD_TRANSFER_PRIORITY_LOW, buffer, len);
  if (frame_num <= 0) {
    return COMMS_STATUS_ERR;
  }
