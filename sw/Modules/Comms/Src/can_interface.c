...
/* canard memory pool */
#define CANARD_MEMORY_POOL_BYTES 1024
static uint8_t memory_pool[CANARD_MEMORY_POOL_BYTES];
...
int16_t canardSTM32Recieve(CAN_HandleTypeDef *hcan, uint32_t RxLocation,
                           CanardCANFrame *const rx_frame) {
  if (rx_frame == NULL) {
  ...
  if (HAL_CAN_GetRxMessage(hcan, RxLocation, &RxHeader, RxData) == HAL_OK) {
    ...
    if (RxHeader.IDE == CAN_ID_EXT) {  // canard will only process the message
                                       // if it is extended ID
    ...
    if (RxHeader.RTR == CAN_RTR_REMOTE) {  // canard won't process the message
  ...
  // Either no CAN msg to be read, or an error that can be read from
  // hfdcan->ErrorCode
  return 0;
}

int16_t canardSTM32Transmit(CAN_HandleTypeDef *hcan,
                            const CanardCANFrame *const tx_frame) {
  if (tx_frame == NULL) {
    return -CANARD_ERROR_INVALID_ARGUMENT;
  }
  ...
  if ((CAN->TSR & CAN_TSR_TME0) || (CAN->TSR & CAN_TSR_TME1) ||
      (CAN->TSR & CAN_TSR_TME2)) {
...
*/
// TODO: All the data in here is temporary for testing. If actually need to send
// valid data, edit accordingly.
...
/*
  get a 16 byte unique ID for this node, this should be based on the CPU unique
  ID or other unique ID
 */
