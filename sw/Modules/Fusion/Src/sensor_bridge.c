...
void can_sensor_reception_setup(){
    (void)cq_init(&imu_tx_queue,
            ...
            IMU_CQ_CAPACITY);
	(void)cq_init(&gps_tx_queue,
            gps_tx_queue_data_block,
            gps_tx_queue_data_lengths,
            GPS_CQ_SLOT_SIZE_BYTE,
            GPS_CQ_CAPACITY);
    ...
    if (dma_busy == 0) {
        if (last_sent_type == 0) {
            ...
            if (cq_pop(&imu_tx_queue, imu_encoded_q, &imu_len) == CQ_OK) {
                HAL_StatusTypeDef ret = HAL_UART_Transmit_DMA(&huart2, imu_encoded_q, imu_len);
                if (ret == HAL_OK) {
                    dma_busy = 1;
            ...
            if (cq_pop(&gps_tx_queue, gps_encoded_q, &gps_len) == CQ_OK) {
                HAL_StatusTypeDef ret = HAL_UART_Transmit_DMA(&huart2, gps_encoded_q, gps_len);
                if (ret == HAL_OK) {
                    dma_busy = 1;
...
void can_sensor_bridge_gps_handler(const struct uavcan_equipment_gnss_SensorGPS* can_gps_p, uint8_t can_id){
	...
	// preparing for the pb data
    struct gps_data_t *pb_gps_p = gps_data_new(encoded, GPS_CQ_SLOT_SIZE_BYTE - GPS_RPROTOBUF_HEADER_LENGTH_BYTE);
	(void)gps_data_time_alloc(pb_gps_p);
	(void)gps_data_date_alloc(pb_gps_p);
	protobuf_pack_GpsData(can_gps_p, pb_gps_p);

	int pb_size = gps_data_encode(pb_gps_p, encoded, GPS_CQ_SLOT_SIZE_BYTE - GPS_RPROTOBUF_HEADER_LENGTH_BYTE);
	...
	// then we add it to the queue, it will be transmit by DMA in main loop
	(void)cq_push(&gps_tx_queue, encode_w_header, pb_size + GPS_RPROTOBUF_HEADER_LENGTH_BYTE);
}
