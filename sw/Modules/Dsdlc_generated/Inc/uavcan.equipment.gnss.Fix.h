...
uint32_t uavcan_equipment_gnss_Fix_encode(struct uavcan_equipment_gnss_Fix *msg,
                                          uint8_t *buffer
...
bool uavcan_equipment_gnss_Fix_decode(const CanardRxTransfer *transfer,
                                      struct uavcan_equipment_gnss_Fix *msg);

#if defined(CANARD_DSDLC_INTERNAL)

static inline void _uavcan_equipment_gnss_Fix_encode(
    uint8_t *buffer, uint32_t *bit_ofs, struct uavcan_equipment_gnss_Fix *msg,
    bool tao);
static inline bool _uavcan_equipment_gnss_Fix_decode(
    const CanardRxTransfer *transfer, uint32_t *bit_ofs,
    struct uavcan_equipment_gnss_Fix *msg, bool tao);
void _uavcan_equipment_gnss_Fix_encode(uint8_t *buffer, uint32_t *bit_ofs,
                                       struct uavcan_equipment_gnss_Fix *msg,
                                       bool tao) {
  (void)buffer;
  (void)bit_ofs;
  ...
  for (size_t i = 0; i < 3; i++) {
    {
      uint16_t float16_val =
          canardConvertNativeFloatToFloat16(msg->ned_velocity[i]);
...
#pragma GCC diagnostic ignored "-Wtype-limits"
  const size_t position_covariance_len =
      msg->position_covariance.len > 9 ? 9 : msg->position_covariance.len;
#pragma GCC diagnostic pop
  for (size_t i = 0; i < position_covariance_len; i++) {
    {
      uint16_t float16_val =
          canardConvertNativeFloatToFloat16(msg->position_covariance.data[i]);
      canardEncodeScalar(buffer, *bit_ofs, 16, &float16_val);
    }
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wtype-limits"
  const size_t velocity_covariance_len =
      msg->velocity_covariance.len > 9 ? 9 : msg->velocity_covariance.len;
#pragma GCC diagnostic pop
  for (size_t i = 0; i < velocity_covariance_len; i++) {
    {
      uint16_t float16_val =
          canardConvertNativeFloatToFloat16(msg->velocity_covariance.data[i]);
      canardEncodeScalar(buffer, *bit_ofs, 16, &float16_val);
    }
...
*/
bool _uavcan_equipment_gnss_Fix_decode(const CanardRxTransfer *transfer,
                                       uint32_t *bit_ofs,
                                       struct uavcan_equipment_gnss_Fix *msg,
                                       bool tao) {
  (void)transfer;
  ...
  if (_uavcan_Timestamp_decode(transfer, bit_ofs, &msg->gnss_timestamp,
                               false)) {
    return true;
  }
  ...
  for (size_t i = 0; i < msg->position_covariance.len; i++) {
    {
      uint16_t float16_val;
      canardDecodeScalar(transfer, *bit_ofs, 16, true, &float16_val);
      msg->position_covariance.data[i] =
  ...
  for (size_t i = 0; i < msg->velocity_covariance.len; i++) {
    {
      uint16_t float16_val;
      canardDecodeScalar(transfer, *bit_ofs, 16, true, &float16_val);
      msg->velocity_covariance.data[i] =
          canardConvertFloat16ToNativeFloat(float16_val);
    }
...
BROADCAST_MESSAGE_CXX_IFACE(uavcan_equipment_gnss_Fix,
                            UAVCAN_EQUIPMENT_GNSS_FIX_ID,
                            UAVCAN_EQUIPMENT_GNSS_FIX_SIGNATURE,
                            UAVCAN_EQUIPMENT_GNSS_FIX_MAX_SIZE);
