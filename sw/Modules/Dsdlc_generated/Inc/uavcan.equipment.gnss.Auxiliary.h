...
uint32_t uavcan_equipment_gnss_Auxiliary_encode(
    struct uavcan_equipment_gnss_Auxiliary *msg, uint8_t *buffer
#if CANARD_ENABLE_TAO_OPTION
    ,
    bool tao
#endif
);
bool uavcan_equipment_gnss_Auxiliary_decode(
    const CanardRxTransfer *transfer,
    struct uavcan_equipment_gnss_Auxiliary *msg);

#if defined(CANARD_DSDLC_INTERNAL)

static inline void _uavcan_equipment_gnss_Auxiliary_encode(
    uint8_t *buffer, uint32_t *bit_ofs,
    struct uavcan_equipment_gnss_Auxiliary *msg, bool tao);
static inline bool _uavcan_equipment_gnss_Auxiliary_decode(
    const CanardRxTransfer *transfer, uint32_t *bit_ofs,
    struct uavcan_equipment_gnss_Auxiliary *msg, bool tao);
void _uavcan_equipment_gnss_Auxiliary_encode(
    uint8_t *buffer, uint32_t *bit_ofs,
    struct uavcan_equipment_gnss_Auxiliary *msg, bool tao) {
  (void)buffer;
  (void)bit_ofs;
...
/*
 decode uavcan_equipment_gnss_Auxiliary, return true on failure, false on
 success
*/
bool _uavcan_equipment_gnss_Auxiliary_decode(
    const CanardRxTransfer *transfer, uint32_t *bit_ofs,
    struct uavcan_equipment_gnss_Auxiliary *msg, bool tao) {
  (void)transfer;
  (void)bit_ofs;
...
struct uavcan_equipment_gnss_Auxiliary
sample_uavcan_equipment_gnss_Auxiliary_msg(void);
#endif
#ifdef __cplusplus
}  // extern "C"
...
BROADCAST_MESSAGE_CXX_IFACE(uavcan_equipment_gnss_Auxiliary,
                            UAVCAN_EQUIPMENT_GNSS_AUXILIARY_ID,
                            UAVCAN_EQUIPMENT_GNSS_AUXILIARY_SIGNATURE,
                            UAVCAN_EQUIPMENT_GNSS_AUXILIARY_MAX_SIZE);
