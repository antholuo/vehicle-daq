...
struct uavcan_equipment_ahrs_RawIMU {
  ...
  int32_t integration_interval;

  int32_t rate_gyro_latest[3];

  int32_t rate_gyro_integral[3];

  int32_t accelerometer_latest[3];

  int32_t accelerometer_integral[3];
...
uint32_t uavcan_equipment_ahrs_RawIMU_encode(
    struct uavcan_equipment_ahrs_RawIMU *msg, uint8_t *buffer
#if CANARD_ENABLE_TAO_OPTION
    ,
    bool tao
#endif
);
bool uavcan_equipment_ahrs_RawIMU_decode(
    const CanardRxTransfer *transfer, struct uavcan_equipment_ahrs_RawIMU *msg);

#if defined(CANARD_DSDLC_INTERNAL)

static inline void _uavcan_equipment_ahrs_RawIMU_encode(
    uint8_t *buffer, uint32_t *bit_ofs,
    struct uavcan_equipment_ahrs_RawIMU *msg, bool tao);
static inline bool _uavcan_equipment_ahrs_RawIMU_decode(
    const CanardRxTransfer *transfer, uint32_t *bit_ofs,
    struct uavcan_equipment_ahrs_RawIMU *msg, bool tao);
void _uavcan_equipment_ahrs_RawIMU_encode(
    uint8_t *buffer, uint32_t *bit_ofs,
    struct uavcan_equipment_ahrs_RawIMU *msg, bool tao) {
  (void)buffer;
  (void)bit_ofs;
  ...
  *bit_ofs += 32;
  ...
  for (size_t i = 0; i < 3; i++) {
    {
      uint16_t float16_val =
          canardConvertNativeFloatToFloat16(msg->rate_gyro_latest[i]);
  ...
  for (size_t i = 0; i < 3; i++) {
    {
      uint16_t float16_val =
          canardConvertNativeFloatToFloat16(msg->accelerometer_latest[i]);
      canardEncodeScalar(buffer, *bit_ofs, 16, &float16_val);
    }
...
#pragma GCC diagnostic ignored "-Wtype-limits"
  const size_t covariance_len =
      msg->covariance.len > 36 ? 36 : msg->covariance.len;
#pragma GCC diagnostic pop
  for (size_t i = 0; i < covariance_len; i++) {
    {
      uint16_t float16_val =
          canardConvertNativeFloatToFloat16(msg->covariance.data[i]);
      canardEncodeScalar(buffer, *bit_ofs, 16, &float16_val);
    }
...
*/
bool _uavcan_equipment_ahrs_RawIMU_decode(
    const CanardRxTransfer *transfer, uint32_t *bit_ofs,
    struct uavcan_equipment_ahrs_RawIMU *msg, bool tao) {
  (void)transfer;
  (void)bit_ofs;
  ...
  *bit_ofs += 32;
  ...
  for (size_t i = 0; i < 3; i++) {
    canardDecodeScalar(transfer, *bit_ofs, 32, true,
                       &msg->rate_gyro_integral[i]);
...
struct uavcan_equipment_ahrs_RawIMU sample_uavcan_equipment_ahrs_RawIMU_msg(
    void);
#endif
#ifdef __cplusplus
}  // extern "C"
...
BROADCAST_MESSAGE_CXX_IFACE(uavcan_equipment_ahrs_RawIMU,
