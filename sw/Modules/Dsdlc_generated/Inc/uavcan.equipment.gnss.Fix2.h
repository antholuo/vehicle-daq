...
uint32_t uavcan_equipment_gnss_Fix2_encode(
    struct uavcan_equipment_gnss_Fix2 *msg, uint8_t *buffer
#if CANARD_ENABLE_TAO_OPTION
    ,
    bool tao
#endif
);
bool uavcan_equipment_gnss_Fix2_decode(const CanardRxTransfer *transfer,
                                       struct uavcan_equipment_gnss_Fix2 *msg);

#if defined(CANARD_DSDLC_INTERNAL)

static inline void _uavcan_equipment_gnss_Fix2_encode(
    uint8_t *buffer, uint32_t *bit_ofs, struct uavcan_equipment_gnss_Fix2 *msg,
    bool tao);
static inline bool _uavcan_equipment_gnss_Fix2_decode(
    const CanardRxTransfer *transfer, uint32_t *bit_ofs,
    struct uavcan_equipment_gnss_Fix2 *msg, bool tao);
void _uavcan_equipment_gnss_Fix2_encode(uint8_t *buffer, uint32_t *bit_ofs,
                                        struct uavcan_equipment_gnss_Fix2 *msg,
                                        bool tao) {
  (void)buffer;
  (void)bit_ofs;
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wtype-limits"
  const size_t ecef_position_velocity_len =
      msg->ecef_position_velocity.len > 1 ? 1 : msg->ecef_position_velocity.len;
#pragma GCC diagnostic pop
  for (size_t i = 0; i < ecef_position_velocity_len; i++) {
    _uavcan_equipment_gnss_ECEFPositionVelocity_encode(
        buffer, bit_ofs, &msg->ecef_position_velocity.data[i], false);
  }
}
...
*/
bool _uavcan_equipment_gnss_Fix2_decode(const CanardRxTransfer *transfer,
                                        uint32_t *bit_ofs,
                                        struct uavcan_equipment_gnss_Fix2 *msg,
                                        bool tao) {
  (void)transfer;
...
#pragma GCC diagnostic pop
  ...
  *bit_ofs += 16;
  ...
  if (!tao) {
    canardDecodeScalar(transfer, *bit_ofs, 1, false,
  ...
  if (tao) {
    ...
    while ((transfer->payload_len * 8) > *bit_ofs) {
      if (_uavcan_equipment_gnss_ECEFPositionVelocity_decode(
              transfer, bit_ofs,
              &msg->ecef_position_velocity
                   .data[msg->ecef_position_velocity.len],
              false)) {
        return true;
      }
      msg->ecef_position_velocity.len++;
...
#pragma GCC diagnostic pop
    for (size_t i = 0; i < msg->ecef_position_velocity.len; i++) {
      if (_uavcan_equipment_gnss_ECEFPositionVelocity_decode(
              transfer, bit_ofs, &msg->ecef_position_velocity.data[i], false)) {
        return true;
      }
    }
  }
...
BROADCAST_MESSAGE_CXX_IFACE(uavcan_equipment_gnss_Fix2,
