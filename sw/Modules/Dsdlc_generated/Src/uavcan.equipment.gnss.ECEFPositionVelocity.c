...
#if CANARD_ENABLE_TAO_OPTION
  if (transfer->tao && (transfer->payload_len >
                        UAVCAN_EQUIPMENT_GNSS_ECEFPOSITIONVELOCITY_MAX_SIZE)) {
    return true; /* invalid payload length */
  }
#endif
  uint32_t bit_ofs = 0;
  if (_uavcan_equipment_gnss_ECEFPositionVelocity_decode(transfer, &bit_ofs,
                                                         msg,
#if CANARD_ENABLE_TAO_OPTION
                                                         transfer->tao
#else
