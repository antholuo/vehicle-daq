...
void angular_rate_from_fs2000dps_to_mdps(int16_t *data_raw,
                                         ImuData_S *imu_data);
...
ImuStatus_E setup_imu_asm330lhh() {
  ...
  asm330lhh_dev_ctx.handle =
      &hspi1;  // TODO: eventualy derive this from an "IMU CONFIGURATION"
  ...
  // Configure IMU
  asm330lhh_device_conf_set(
      &asm330lhh_dev_ctx,
      PROPERTY_ENABLE);  // Set device configuration (no clue what this does)
  asm330lhh_block_data_update_set(&asm330lhh_dev_ctx,
                                  PROPERTY_ENABLE);  // Enable block data update
  asm330lhh_xl_data_rate_set(
      &asm330lhh_dev_ctx,
      ASM330LHH_XL_ODR_417Hz);  // TODO: determine the correct output rates
  asm330lhh_gy_data_rate_set(&asm330lhh_dev_ctx, ASM330LHH_GY_ODR_417Hz);
  asm330lhh_xl_full_scale_set(
      &asm330lhh_dev_ctx, ASM330LHH_2g);  // TODO: determine the correct scaling
  asm330lhh_gy_full_scale_set(&asm330lhh_dev_ctx, ASM330LHH_2000dps);
  asm330lhh_timestamp_set(&asm330lhh_dev_ctx,
                          PROPERTY_ENABLE);  // Enable timestamping
...
ImuStatus_E poll_imu_asm330lhh(ImuData_S *imu_data) {
  ...
  if (status_reg.xlda || status_reg.gda) {
    ...
    // if there is data (something new) and our TS changes ,invalidated all the
    // data.
...
int32_t platform_read(void *handle, uint8_t reg, uint8_t *bufp, uint16_t len) {
  ...
  // This SPI pin is currently hardcoded, eventually we want to add this to some
