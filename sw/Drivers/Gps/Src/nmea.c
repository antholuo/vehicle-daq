 ...
 *      Author: antho
 *    Modified: 2025-MAY-12 (copied to Drivers/)
 *
...
//   buf - pointer to the pointer to the data buffer
// return: parsed value
// note: buf will point to the next byte after parsed value or to the next term
// if
//       a byte after parsed number is ','
// note: the function will parse the buffer contents until it hits a non-digit
// char,
...
int32_t atoi_chr(uint8_t **buf) {
  ...
  // Parse buffer until first non-digit character, no overflow check for
  // 'value', therefore 32-bit value will overflow if number length is more than
  // 10 digits
...
uint8_t atoi_hex(uint8_t *buf) {
  ...
  // If byte contains letter, it will be converted to lowercase, in case of
  // digit the byte will remain intact
  tmp = (*buf++ | 0x20);
...
// if
//       a byte after parsed number is ','
// note: float will be represented as integer, e.g. string '1234.567' will be
// parsed to 1234567
...
// note: the result pointer will point to the last byte of a sentence or to the
// end
...
// note: buf pointer will point to the next term or to the sentence end ('*'
// character)
...
void NMEA_FindSentence(NmeaSentence_T *sentence, uint8_t *buf_start,
  ...
  // Further parsing does not make sense if there is no space left in buffer for
  // NMEA sentence
...
void NMEA_ParseLatLon(uint8_t **buf, uint8_t deg_len, int32_t *value,
  ...
  if (**buf != ',') {
    ...
    *value += atoi_len(buf, 2);
    ...
    // If a fractional part consists more than 4 digits, need to scan up to the
    // end of the number
    if (**buf != ',') {
    ...
    // value of '10000' - the scaling factor, deending on the length of the
    // fractional part
  ...
  if (**buf != ',') {
    ...
    *buf += 2;
    ...
    // In case of 'S' latitude or 'W' longitude the degrees value must be
    // negative
...
void NMEA_ParseRmc(uint8_t *ptr) {
  ...
  if (*ptr != ',') {
    ...
    if (data_RMC.nmea_time.date_utc.year > 69) {
      ...
      // Copy fix_date to date and fix_time to time in case of the $GPZDA
      // sentence are disabled
...
void NMEA_ParseSentence(NmeaSentence_T *sentence) {
  ...
  switch (sentence->type) {
    case NMEA_xxRMC:
      // This is the case we care the most about, since it gives us basically
      // all the data we need. This typically comes through as a GN message.
      NMEA_ParseRmc(ptr);
      break;
    case NMEA_xxZDA:
...
void NMEA_ParseBuf(uint8_t *buf, uint16_t *length) {
  ...
  while (ptr < buf_end) {
    ...
    if (ptr[0] == '$') {  // Found the start of a sentence
      ...
      switch (hdr) {  // Not sure why this was easier than doing a strcmp...but
                      // here we are
