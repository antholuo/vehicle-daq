...
typedef struct {
  ...
  int32_t lat_microdeg;  // latitude, microdegrees. Sign indicates direction
                         // (+n, -s)
  int32_t lon_microdeg;  // longitude, microdegrees. Sign indicates direction
                         // (+e, -w)
  ...
  uint8_t fix_status;  // 0 = No gps, 1 = GPS Fix, 2 = DGSP Fix, 3 = Estimated /
                       // Dead Reckoning
  bool data_valid;
} GpsData_S;
...
bool gnss_parse_data_if_available(const GpsType_T *const gps_type,
                                  const size_t num_gps, GpsData_S *gps_data);
