 ...
 *      Author: anni
 *    Modified: 2025-MAY-10 (copied to new location)
 *
...
#define MAX_SATELLITES \
  12  // maximum number of satellites the gps receiver can handle
...
enum NmeaSentenceType_E {
  ...
  NMEA_xxGNS,  // GNS = GNSS fix data, incl. Time, Pos, Num satellites, HDOP,
               // age of differential data, etc.
  ...
  NMEA_xxTHS,  // THS = True Heading Status -- we probably don't have this since
               // we don't hve a compass
...
// TODO: rest of the structs. We're going to bring this on ONE AT A TIME!!!!
// slow and steady...
typedef struct {
  ...
  uint8_t quality;  // 0 = no fix, 1 = autonomous Gnss, 2 = differential gnss, 4
                    // = rtk fied, 5 = rtk float, 6 = estimated/dead reckoning
  ...
  uint8_t
      diff_age;  // Age of differential corrections (Null when not using DGPS)
  uint8_t diff_station;  // ID of station providing differential corrections
                         // (null when not used)
...
typedef struct {
  ...
  bool data_valid;  // *Technically a character for data validity, but we only
                    // care about t/f
  int32_t lat_microdeg;  // latitude in microdegrees
  uint8_t lat_char;      // latitude direction indicator (N/S)
  int32_t lon_microdeg;  // longitude in microdegrees
  ...
  uint8_t pos_mode;  // Position mode. A=autonomous, D=differential,
                     // E=estimated, R=coarse, S=simulator, N=not valid
} RmcData_T;
